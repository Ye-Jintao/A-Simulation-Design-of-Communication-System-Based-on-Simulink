...
char_count_values = [10, 100, 1000, 10000, 20000, 30000, 40000,50000,100000];  % 需要遍历的字符数量
WER = zeros(1, length(char_count_values)); 
for n = 1:length(char_count_values)
    % 只读取文件中的前 n 个字符
    fileID = fopen(filename, 'r');
    input_data = fread(fileID, char_count_values(n), '*char')';  % 读取整个文件作为字符数组
    fclose(fileID);

    % 步骤 2: 仅保留ASCII字符（这里没使用，你可以解除注释过滤非ASCII字符）
...
% 步骤 11: 绘制误码率与字符数量的关系
figure;
semilogx(char_count_values, WER, '-o', 'LineWidth', 2);  % 使用对数尺度显示X轴
title('误码率（WER）与字符数量的关系');
xlabel('字符数量 (Log Scale)');
ylabel('误码率 (WER)');
grid on;
