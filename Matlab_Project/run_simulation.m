...
% 步骤 10:将解码后的数据写入文件
output_filename = 'decoded_output.txt';  % 输出文件名
fileID = fopen(output_filename, 'w');  % 打开文件以进行写入
% 将解码数据转换为字符并写入文件
fprintf(fileID, '%s', char(decoded_data));  % 假设解码的数据是字符数据
% 关闭文件
fclose(fileID);
...
% 输出误码率
disp(['Word Error Rate (WER) = ', num2str(word_error_rate)]);